...
window.onload = () => {
    ...
    window.matchMedia('screen and (orientation:portrait)').addEventListener("change", e => handleOrientation(e));
    ...
    activeRegion.bind(containerElement, swipe, function (event) {
        if (moveMode) {
            ...
            rotationLog.innerText = "Rotation: " + rotation.x.toFixed() + ', ' + rotation.y.toFixed() + ', ' + rotation.z.toFixed();
        }
    });

    // Set the willReadFrequently attribute for A-Frame canvas
    let aframeCanvas = document.querySelector('canvas.a-canvas');
    if (aframeCanvas) {
        aframeCanvas.willReadFrequently = true;
    }
};
